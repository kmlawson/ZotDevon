...
This script is not designed to replace your Zotero collection with the same data in DEVONthink, it is designed for those of us who want to use both tools hand in hand, using Zotero as a citation manager and home for our PDFs, while using DEVONthink to organize our notes.

# What ZotDevon Does Not Currently Do

1. ZotDevon is one-way and can only add files not found in the local index file (keys.txt). It does not delete DEVONthink items that are no longer found in your Zotero. It does not re-add items that were deleted in DEVONthink but which are still listed in the local index.
...
1. Install [ASObjC Runner](http://www.macosxautomation.com/applescript/apps/runner.html). Download and place it in your Applications forlder. This is used to give you feedback on how the import process is going.
...
4. Open the ZotDevon.scpt file in AppleScript Editor and configure the script for your machine. Tell it where you put the ZotDevon folder, what folder in DEVONthink you want to import items into.
5. Login to Zotero and [request a key](https://www.zotero.org/settings/keys) so the script can access your library. While you are there, make a note of your "userID"
...
9. Back in DEVONthink, make sure the desired database is active, and select  ZotDevon from the applescript menu. In tests, took under 3 hours the first time you import a Zotero library of 2500 (5300 total items including attachments).
...
1. **The Import Failed and the Progess Window Won't Close**

Open Activity Monitor in your Utilities folder, search for ASOBjC Runner, and force quit it.

2. **I Want to Reimport Everything From Scratch**
